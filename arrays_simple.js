...
const customersSalary = [
  ...
  function userSortedByFirstName(){
    return (userSorted.map(us => us.name["first"])).sort();
}
//  console.log(userSortedByFirstName());
  

  // 2. Calculate the total salary of all users and log the overall amount. (customersSalary)
function customersTotalSalary() {
  let sum = customersSalary.reduce((sum, user) => sum + +user.salary, 0);
  return  +sum.toFixed(0);
}
// console.log(customersTotalSalary());

  
  // 3. Create a new array that filters out all users whose friend count is less than 8. (filterUser)
  function filterUserByFriendCount() {
    return filterUser.filter(user => user.friends.length < 8);
   
  }
  // console.log(filterUserByFriendCount());
  
  // 4. Map over the user array to collect all friends into a single array. (mappedUsers)
  function collectFriendsIntoASingleArray() {
    return mappedUsers.flatMap(user => user.friends);
  }
  
  // console.log(collectFriendsIntoASingleArray());
  ...
  function aggregateData() {
    return mappedUsers.map(user => [{id: user._id, name: user.name, isActive: user.isActive}]);
  }
